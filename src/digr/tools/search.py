...
def _discover_unlabelled(
    ...
    for path, library_name, duration, fits in staged:
        if decoded_count >= DETECTION_BUDGET_FILES or time.monotonic() >= deadline:
            break
        considered_count += 1
        # target.high is the fastest tempo this search could admit, so it
        # gives the shortest bar length a genuine candidate could have here --
        # anything under that can't be >= 1 bar at any tempo in range.
